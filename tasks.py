...
def zadanieB():
    ...
    plt.ylabel("Wartość norm residuum")
...
def zadanieD():
    ...
    A = create_A(size, -3, -1, -1)
    b = create_b(size)
    _, residuum, time = solve_LU(A, b)
    print("Zadanie D")
    print("LU Residdum = " + str(residuum))
    print("LU Time = " + str(time))
...
def zadanieE():
    ...
    for i in size:
        ...
        A = create_A(i, 12, -1, -1)
        b = create_b(i)
        _, _, jacoby_t, jacoby_r, _ = jacoby(A, b)
        _, _, gauss_t, gauss_r, _ = gauss_seidler(A, b)
